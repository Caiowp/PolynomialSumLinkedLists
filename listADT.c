...
void printList(LIST* list) {
    ...
    while (current != NULL) {
    	if (*current->expo > 0) printf("+ %dx^%d ", *current->coeff, *current->expo);
    	if (*current->expo < 0) printf("%dx^%d ", *current->coeff, *current->expo);
    	if (*current->expo == 0 ) printf(" %d ", *current->coeff);
 ...
 * Post head has allocated or error returned Return head node pointer or null if overflow*/
...
int addNode (LIST* pList, int* coeff, int* expo) {
	...
	//Statements
	found = _search (pList, &pPre, &pLoc, expo);
	if (found){
		// Add to duplicate exponent
		*(pLoc->coeff) = *(pLoc->coeff)+*(coeff);
		return (1);
	}
	success = _insert (pList, pPre, coeff, expo);
	if (!success)
		// Overflow
		return (-1);
	return (0);
 ...
 * Pre pList pointer to a valid list p
 * Pre pointer to data's predecessor dataInPtr data pointer to be inserted
 * Post data have been inserted in sequence Return boolean, true if successful, false if memory overflow*/
static bool _insert (LIST* pList, NODE* pPre, int* coeff, int* expo) {
	//Local Definitions
	NODE* pNew;
	...
	if (pPre == NULL) {
		...
		pNew->link = pPre->link;
		pPre->link = pNew;
		// Now check for add at end of list ƒ
		if (pNew->link == NULL)
				pList->rear = pNew;
	} // if else
 ...
 * Pre pList pointer to a valid list keyPtr pointer to key to be deleted dataOutPtr pointer to data pointer
...
bool removeNode (LIST* pList, int* keyPtr, int** dataOutPtr) {
	...
	//Statements
	found = _searchCoeff (pList, &pPre, &pLoc, keyPtr);
	if (found)
 ...
 * pPre pointer to predecessor node pLoc pointer to target node dataOutPtr pointer to data pointer
 * Post Data have been deleted and returned Data memory has been freed */
void _delete (LIST* pList, NODE* pPre, NODE* pLoc, int** dataOutPtr) {
	//Statements
	*dataOutPtr = pLoc->expo;
 ...
 * Post
 * 		pLoc points to first equal/greater key -or- null if target > key of last node
...
static bool _search (LIST* pList, NODE** pPre, NODE** pLoc, int* pArgu) {
	//Macro Definition
	#define COMPARE \
		( ((* pList->compare) (pArgu, (*pLoc)->expo)) )
	#define COMPARE_LAST \
		((* pList->compare) (pArgu, pList->rear->expo))
		//Local Definitions
		int result;
		//Statements
		*pPre = NULL;
...
static bool _searchCoeff (LIST* pList, NODE** pPre, NODE** pLoc, int* pArgu) {
	//Macro Definition
	#define COMPARECOEFF \
		( ((* pList->compare) (pArgu, (*pLoc)->coeff)) )
	#define COMPARECOEFF_LAST \
		((* pList->compare) (pArgu, pList->rear->coeff))
		//Local Definitions
		int result;
		//Statements
		*pPre = NULL;
		*pLoc = pList->head;
		if (pList->count == 0)
			return false;
		// Test for argument > last node in list
		if ( COMPARECOEFF_LAST > 0) {
			*pPre = pList->rear;
			*pLoc = NULL;
			return false;
		} // if

		while ( (result = COMPARECOEFF) > 0 ) {
			// Have not found search argument location
			*pPre = *pLoc; *pLoc = (*pLoc)->link;
		} // while
		if (result == 0)
...
bool traverse (LIST* pList, int fromWhere, int** dataPtrOut) {
	//Statements
	if (pList->count == 0)
		return false;
	if (fromWhere == 0) {
...
LIST* destroyList (LIST* pList) {
	...
	if (pList) {
		while (pList->count > 0) {
			// First delete data
			free (pList->head->expo);
			free (pList->head->coeff);
