...
void printList(LIST* list) {
    ...
    while (current != NULL) {
    	if (*current->expo > 0) printf("+ %dx^%d ", *current->coef, *current->expo);
    	if (*current->expo < 0) printf("%dx^%d ", *current->coef, *current->expo);
    	if (*current->expo == 0 ) printf(" %d ", 1);
...
int addNode (LIST* pList, int* coef, int* expo) {
	//Local Definitions
	bool found;
	bool success;
	NODE* pPre;
	...
	if (found){
		...
		*(pLoc->coef) = *(pLoc->coef)+*(coef);
		return (1);
	}
	success = _insert (pList, pPre, coef, expo);
	if (!success)
		// Overflow
		return (-1);
	return (0);
...
static bool _insert (LIST* pList, NODE* pPre, int* coef, int* expo) {
	//Local Definitions
	NODE* pNew;
